...
PORT           = 1883
TOPIC_SERVO    = "petbuddy/sofia/servo"       
TOPIC_CONFIRM  = "petbuddy/sofia/confirmacion"
...
/* Oculta el logo feo de Streamlit */
...
# ─────────────────────────────────────────
st.markdown(
    '<p class="pet-title">🐶 Alimentar mascota</p>',
    unsafe_allow_html=True
)

st.markdown(
    '<p class="pet-subtitle">Control inteligente del dispensador de comida</p>',
    unsafe_allow_html=True
)
...
with col_control:
    ...
    st.markdown(
        '<div class="card-title">Control del dispensador</div>',
        unsafe_allow_html=True
    )
    ...
    if st.session_state.ultimo_feed:
        st.caption(
            f"Última alimentación: {st.session_state.ultimo_feed}"
        )
...
with col_count:
    ...
    st.markdown(
        ...
        <div class="feed-count">
            {st.session_state.veces_alimentado}
        </div>

        <div class="feed-label">
            veces alimentado hoy
        </div>
        ''',
        unsafe_allow_html=True
    )
...
with col_info:
    ...
    4. El servo gira y dispensa comida  
    5. El ESP32 responde `ok`  
    """)
...
st.caption(
    "🐶 PetBuddy · Alimentador automático · MQTT + Servo + ESP32"
)
